# 🎬 IMDb-Analysis

This project focuses on analyzing movie reviews data from IMDb, utilizing various data warehousing, mining, and visualization techniques to gain insights into movie trends.

## Project Structure

- **📁 Data/**: Contains the dataset used for analysis.
- **📓 Jupyter Notebook/**: Includes notebooks for data cleaning, preprocessing, and mining tasks.
- **📄 Presentation/**: Project presentation materials.
- **📄 Report/**: Detailed project report and findings.

## Key Features
- **Data Extraction and Cleaning**: Performed using Jupyter Notebooks.
- **Data Warehousing**: Managed using Microsoft SQL Server.
- **OLAP Cube Creation**: Built with Visual Studio.
- **Data Visualization**: Created using Power BI for visual insights.

## Technologies Used
- **Python**: Pandas for data manipulation, analysis, and visualization.
- **Microsoft SQL Server**: For data warehousing and ETL processes.
- **Visual Studio**: Used for OLAP cube creation.
- **Tableau**: For interactive dashboards and data visualization.

## Contributors
- **Alexandra Stathopoulou**
